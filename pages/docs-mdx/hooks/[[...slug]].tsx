...
export default function Page({ doc }: PageProps) {
  ...
  return (
    <>
      ...
      <DocumentLayout title={doc.title} category="hooks">
        <MDXContent />
      </DocumentLayout>
    </>
  )
...
const matchesThisRoute = (doc: DocMdx) => doc._raw.sourceFileDir.startsWith('docs-mdx/hooks');
const compareAsJSON = (a: unknown, b: unknown) => JSON.stringify(a) === JSON.stringify(b);
const buildSlug = (doc: DocMdx) => doc._raw.flattenedPath.replace('docs-mdx/hooks/', '').split('/');
